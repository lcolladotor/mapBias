...
geu_bam <- file.path('..', 'geuvadis', 'tophat', tophat, geu_samples, 
    'accepted_hits.bam')
sim_samples <- paste0('sample0', 1:7)
sim_dir <- rep(c('default_1', 'default_2', 'rnaf_1', 'rnaf_2'), each = 7)
sim_bam <- file.path('..', 'tophat', tophat, sim_dir, sim_samples, 
    'accepted_hits.bam')
...
## Load coverage
fullCov <- fullCoverage(gff, chrs = levels(seqnames(regions)), which = regions, 
    protectWhich = 1e4, mc.cores = 7)
...
plotRegionCoverage(regions, regCov, groupInfo, annoNear, annoReg, txdb = txdb, scalefac = 1, ask = FALSE)
```
...
for(i in seq_len(7)) {
    ...
    plotRegionCoverage(regions, sub_regCov, sub_groupInfo, annoNear, annoReg, txdb = txdb, scalefac = 1, ask = FALSE)
}
```
...
for(i in seq_len(7)) {
    ...
    for(j in seq_len(length(sub_regCov))) {
        ...
        ggpairs(tmp, upper = list(continuous = 'smooth'), lower = list(continuous = 'cor'), title = names(sub_regCov)[j])
    }
}
```
...
plotRegionCoverage(exons, ex_regCov, groupInfo, ex_annoNear, ex_annoReg, txdb = txdb, scalefac = 1, ask = FALSE)
```
...
for(i in seq_len(7)) {
    ...
    plot.new()
    text(0.5, 0.5, geu_samples[i], cex = 5)
    plotRegionCoverage(exons, sub_regCov, sub_groupInfo, ex_annoNear, ex_annoReg, txdb = txdb, scalefac = 1, ask = FALSE)
}
```
